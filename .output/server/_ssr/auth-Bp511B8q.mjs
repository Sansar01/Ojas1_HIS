...
async function apiRequest(path, options = {}) {
	...
	const makeRequest = async (token) => {
		...
		return fetch(buildUrl(path, params), {
			...
			credentials: "include",
			body: body !== void 0 && !(body instanceof FormData) ? JSON.stringify(body) : body
		});
	};
	...
	if (!response.ok) throw new ApiError(`Request failed with status ${response.status}`, response.status, payload);
	return payload;
}
var api = {
...
async function refreshAccessToken() {
	...
	refreshPromise = (async () => {
		try {
			const response = await fetch(`${DEFAULT_BASE_URL}/api/hospital/auth/refresh`, {
				method: "POST",
				credentials: "include"
			});
...
async function loginWithBackend(email, password) {
	...
	if (!response || !payload) throw new Error(payload?.accessToken ? "Login failed" : "Unable to sign in with the provided credentials.");
	const user = {
...
//#endregion
export { logOutFromFrontend as a, getUser as i, api as n, loginWithBackend as o, canAccess as r, setUser as s, ApiError as t };
