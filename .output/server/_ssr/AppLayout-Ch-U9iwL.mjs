...
import { a as logOutFromFrontend, i as getUser, n as api, r as canAccess } from "./auth-Bp511B8q.mjs";
...
function useRoles() {
	...
	(0, import_react.useEffect)(() => {
		api.get("/api/hospital/roles").then(setRoles).catch((e) => setError(e.message)).finally(() => setLoading(false));
	}, []);
	return {
		roles,
...
function useDepartments() {
	...
	const [error, setError] = (0, import_react.useState)(null);
	(0, import_react.useEffect)(() => {
		api.get("/api/hospital/masters/departments", { params: { active: true } }).then(setDepartments).catch((e) => setError(e.message)).finally(() => setLoading(false));
	}, []);
	return {
		departments,
		loading,
...
function useShifts() {
	...
	const [error, setError] = (0, import_react.useState)(null);
	(0, import_react.useEffect)(() => {
		api.get("/api/hospital/masters/shifts", { params: { active: true } }).then(setShifts).catch((e) => setError(e.message)).finally(() => setLoading(false));
	}, []);
	return {
		shifts,
		loading,
...
function useRolePermissions(roleId) {
	...
	(0, import_react.useEffect)(() => {
		...
		setLoading(true);
		setError(null);
		api.get(`/api/hospital/roles/${roleId}/permissions`).then(setPermissions).catch((e) => setError(e.message)).finally(() => setLoading(false));
	}, [roleId]);
	return {
		permissions,
		loading,
...
function useHospitalUsers() {
	...
	const [error, setError] = (0, import_react.useState)(null);
	(0, import_react.useEffect)(() => {
		api.get("/api/hospital/users", { params: { status: "ACTIVE" } }).then((data) => {
			if (!Array.isArray(data)) {
				setUsers([]);
				return;
			}
